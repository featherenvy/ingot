...
impl JobDispatcher {
    #[allow(dead_code)]
    pub(super) async fn tick_system_action(&self) -> Result<bool, RuntimeError> {
    ...
    pub(super) async fn promote_queue_heads(
        &self,
    ...
    pub(super) async fn auto_finalize_prepared_convergence(
        &self,
        project_id: ingot_domain::ids::ProjectId,
        ...
        let target_valid = current_target_oid.as_deref() == Some(prepared_commit_oid.as_str())
            || current_target_oid.as_deref() == Some(input_target_commit_oid.as_str());
        if !target_valid {
            return Ok(false);
        }
    ...
    pub(super) async fn invalidate_prepared_convergence(
        &self,
        project_id: ingot_domain::ids::ProjectId,
        item_id: ingot_domain::ids::ItemId,
    ) -> Result<(), RuntimeError> {
    ...
    #[allow(clippy::too_many_arguments)]
    pub(super) async fn prepare_queue_head_convergence(
        &self,
        project: &Project,
        item: &ingot_domain::item::Item,
        revision: &ItemRevision,
    ...
    pub(super) async fn reconcile_checkout_sync_state(
        &self,
        project: &Project,
        item_id: ingot_domain::ids::ItemId,
        revision: &ItemRevision,
