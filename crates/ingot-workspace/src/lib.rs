...
async fn reset_existing_worktree(
    ...
    git(
        workspace_path,
        &["checkout", "--detach", "--force", expected_head_oid],
    )
    .await?;
    git(workspace_path, &["reset", "--hard", expected_head_oid]).await?;
...
mod tests {
    ...
    async fn provision_authoring_workspace_resets_existing_worktree_to_expected_head() {
        ...
        provision_authoring_workspace(&repo, &workspace_path, workspace_ref, &base_head)
            .await
            .expect("first provision");
    ...
    async fn provision_integration_workspace_resets_existing_worktree_to_expected_head() {
        ...
        provision_integration_workspace(&repo, &workspace_path, workspace_ref, &base_head)
            .await
            .expect("first provision");
